"""add_creator_submission_model

Revision ID: 0034103f8b37
Revises: 1640492a1506
Create Date: 2025-12-26 21:38:38.956741
...
revision: str = '0034103f8b37'
down_revision: Union[str, Sequence[str], None] = '1640492a1506'
...
def upgrade() -> None:
    ...
    sa.Column('pattern_id', sa.String(length=200), nullable=False),
    sa.Column('pattern_library_id', sa.UUID(), nullable=True),
    sa.Column('user_id', sa.UUID(), nullable=False),
    sa.Column('video_url', sa.String(length=500), nullable=False),
    sa.Column('platform', sa.String(length=50), nullable=False),
    sa.Column('creator_notes', sa.Text(), nullable=True),
    sa.Column('invariant_checklist', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('status', sa.Enum('PENDING', 'TRACKING', 'COMPLETE', 'FAILED', name='creatorsubmissionstatus'), nullable=False),
    ...
    sa.Column('performance_vs_baseline', sa.String(length=20), nullable=True),
    sa.Column('submitted_at', sa.DateTime(), nullable=False),
    sa.Column('tracking_started_at', sa.DateTime(), nullable=True),
    sa.Column('tracking_completed_at', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['outlier_item_id'], ['outlier_items.id'], ),
    sa.ForeignKeyConstraint(['pattern_library_id'], ['pattern_library.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_creator_submissions_pattern_id'), 'creator_submissions', ['pattern_id'], unique=False)
    ...
    op.alter_column('notebook_source_packs', 'temporal_phase',
               existing_type=sa.VARCHAR(length=20),
               nullable=False)
    op.drop_index(op.f('idx_notebook_source_packs_cluster_id'), table_name='notebook_source_packs')
    op.alter_column('outlier_items', 'analysis_status',
               existing_type=sa.VARCHAR(length=20),
               nullable=False,
               existing_server_default=sa.text('NULL::character varying'))
    op.create_foreign_key(None, 'outlier_items', 'users', ['approved_by'], ['id'])
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, 'outlier_items', type_='foreignkey')
    op.alter_column('outlier_items', 'analysis_status',
               existing_type=sa.VARCHAR(length=20),
               nullable=True,
               existing_server_default=sa.text('NULL::character varying'))
    op.create_index(op.f('idx_notebook_source_packs_cluster_id'), 'notebook_source_packs', ['cluster_id'], unique=False)
    op.alter_column('notebook_source_packs', 'temporal_phase',
               existing_type=sa.VARCHAR(length=20),
               nullable=True)
    ...
    op.drop_index(op.f('ix_creator_submissions_pattern_id'), table_name='creator_submissions')
