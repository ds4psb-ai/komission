"""
VDG (Video Data Graph) v3.5
Gemini 3 Pro Structured Output Compatible Version

Changes from v3.3:
- Replaced ALL Dict[str, X] fields with typed object structures
- Gemini API does not support additionalProperties (dynamic keys)
- 100% feature parity with v3.3, just different field representation
"""
...
# ==================== FOCUS WINDOW (v3.5 - No Dict) ====================
class HotspotScores(BaseModel):
    """Replaced Dict[str, float] with typed fields"""
    hook: float = 0.0
    interest: float = 0.0
    boundary: float = 0.0

class SourceEvidence(BaseModel):
    """Replaced Dict[str, Any] with typed fields"""
    cue_type: str = ""
    description: str = ""
...
class CompositionSpec(BaseModel):
    """Replaced Dict[str, Any] → typed object"""
    grid: str = "center"
    subject_size: str = "MS"
...
# ==================== CROSS-SCENE ANALYSIS (v3.5 - No Dict) ====================
...
# ==================== MAIN VDG MODEL ====================
class VDGv35(BaseModel):
    """
    VDG (Video Data Graph) v3.5
    Gemini 3 Pro Structured Output Compatible Version
    
    v3.5 Changes from v3.3:
    - All Dict[str, X] fields converted to typed object structures
    - Gemini API does not support additionalProperties (dynamic keys)
    - 100% feature parity with v3.3
    - global_context and scene_frames removed (use dedicated typed fields)
    """
    # Identity
    content_id: str
    ...
    # Focus Windows (RL reward signals)
    focus_windows: List[FocusWindow] = Field(default_factory=list)
    
    # Cross-Scene Analysis (pattern synthesis)
