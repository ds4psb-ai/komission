# 코미션 (Komission) 개발자 온보딩 가이드

> **최종 수정**: 2026-01-02 (모바일 앱 하드닝 완료 반영)

## 🎯 프로젝트 개요

**코미션**은 틱톡 크리에이터를 위한 AI 기반 영상 촬영 코칭 플랫폼입니다.

### 핵심 기능
1. **아웃라이어 분석** - 바이럴 틱톡 영상 분석 (VDG 기반)
2. **실시간 AI 코칭** - 촬영 중 음성/텍스트 피드백
3. **체험단 (캠페인)** - 크리에이터가 브랜드 캠페인에 참여

---

## 🏗️ 아키텍처

```
┌─────────────────────────────────────────────┐
│                   BACKEND                    │
│         FastAPI + PostgreSQL + Redis         │
│    /Users/ted/komission/backend              │
│                                              │
│  ┌─────────────────────────────────────┐    │
│  │ WebSocket: /api/v1/ws/coaching/{session_id}      │◄───┼──── 웹/앱 공통 사용
│  │ REST API: /api/v1                   │    │
│  └─────────────────────────────────────┘    │
└─────────────────────────────────────────────┘
           │                    │
           ▼                    ▼
┌──────────────────┐   ┌──────────────────┐
│    WEB APP       │   │   MOBILE APP     │
│    Next.js       │   │ Expo + RN        │
│  /frontend       │   │ /mobile ✅       │
│                  │   │                  │
│ • 아웃라이어     │   │ ✅ 4K 촬영       │
│ • 체험단 관리    │   │ ✅ H.265/H.264   │
│ • 캔버스         │   │ ✅ 코칭 오버레이 │
│ • 대시보드       │   │ ✅ 음성/텍스트   │
└──────────────────┘   └──────────────────┘
```

---

## 📂 프로젝트 구조

```
/Users/ted/komission/
├── backend/                    # FastAPI 백엔드
│   ├── app/
│   │   ├── main.py
│   │   ├── routers/            # 33 files
│   │   │   ├── coaching_ws.py  # ⭐ 실시간 코칭 WebSocket (1124 lines)
│   │   │   ├── coaching.py     # 세션 REST API (586 lines)
│   │   │   ├── outliers.py     # 아웃라이어 (106KB)
│   │   │   ├── agent.py        # Chat Agent
│   │   │   └── o2o.py          # 체험단
│   │   ├── services/           # 50+ files
│   │   │   ├── vdg_2pass/      # VDG 분석 엔진 (16 files)
│   │   │   ├── audio_coach.py  # Gemini 2.5 Flash (30KB)
│   │   │   ├── coaching_repository.py  # DB 레포지토리 (33KB)
│   │   │   └── frame_analyzer.py
│   │   ├── mcp/                # MCP 서버
│   │   │   ├── tools/          # MCP tools (VDG + STPF)
│   │   │   ├── resources/      # MCP resources (komission + STPF)
│   │   │   └── prompts/        # MCP prompt templates
│   │   ├── models.py           # CoachingSession, Intervention 등 (2036 lines)
│   │   └── schemas/            # 17 files
│   └── requirements.txt
│
├── frontend/                   # Next.js 웹앱
│   ├── src/
│   │   ├── app/                # App Router
│   │   ├── components/         # 29 files + 7 subdirs
│   │   │   ├── CoachingSession.tsx  # ⭐ 코칭 UI (45KB, 979 lines)
│   │   │   ├── PatternAnswerCard.tsx
│   │   │   └── EvidenceBar.tsx
│   │   └── hooks/
│   │       └── useCoachingWebSocket.ts
│   └── package.json
│
├── mobile/                     # ✅ Expo 앱 (Week 1 완료)
│   ├── app.json
│   ├── package.json
│   ├── app/                    # expo-router
│   │   ├── _layout.tsx
│   │   ├── index.tsx
│   │   └── camera.tsx          # ⭐ 4K 촬영 (550+ lines)
│   └── src/
│       ├── config/
│       │   └── recordingConfig.ts   # H.265/H.264 (236 lines)
│       ├── hooks/              # 4 hooks
│       │   ├── useCoachingWebSocket.ts  # Phase 2 스트리밍 (13KB)
│       │   ├── useCameraFormat.ts       # 프레임 레이트 (7KB)
│       │   ├── useDeviceStatus.ts       # 배터리/네트워크 (8KB)
│       │   └── useSessionPersistence.ts # DB 연동 (10KB)
│       ├── services/
│       │   └── videoStreamService.ts  # H.264 + AdaptiveBitrate
│       └── components/         # 4 components
│           ├── CoachingOverlay.tsx  # ⭐ 음성/텍스트 토글 (14KB)
│           ├── RecordButton.tsx     # 녹화 버튼 애니메이션
│           ├── QualityBadge.tsx     # 4K/HEVC 배지
│           └── DeviceStatusBar.tsx  # 디바이스 상태
│
└── docs/                       # 문서
    ├── 21_PARALLEL_DEVELOPMENT_STRATEGY.md
    ├── 22_DEVELOPER_ONBOARDING.md
    ├── ARCHITECTURE_FINAL.md
    └── CHANGELOG.md
```

---

## 🔀 병렬 개발 트랙

### 트랙 A: 모바일 앱 ✅ (Week 1 완료)

**담당**: Claude (AI 어시스턴트)

| 항목 | 상태 |
|------|------|
| 4K 촬영 + H.265 | ✅ 완료 |
| 프레임 레이트 안정화 | ✅ 완료 |
| 배터리/네트워크 적응 | ✅ 완료 |
| H.264 스트리밍 최적화 | ✅ 완료 |
| 음성/텍스트 토글 | ✅ 완료 |
| DB 세션 저장 | ✅ 완료 |
| 딥링크 연결 | ⬜ 대기 |
| 앱스토어 등록 | ⬜ 대기 |

---

### 트랙 B: 웹앱 고도화 ← **새 개발자 담당**

| 항목 | 내용 |
|------|------|
| **목표** | 코칭 품질 향상 + 체험단 기능 |
| **기술** | Next.js 16 + Tailwind (next-intl i18n) |
| **폴더** | `/frontend` |

#### Phase 1: 코칭 품질 향상
- [ ] CV 메트릭 기반 실시간 코칭 (center_offset, brightness, blur)
- [ ] VDG shotlist/do_not 기반 맞춤 피드백
- [ ] 코칭 메시지 다양성 증가

#### Phase 2: 체험단 고도화
- [ ] 캠페인 생성/관리 UI
- [ ] 크리에이터 신청/선발 플로우
- [ ] 영상 제출 및 검수 시스템
- [ ] 보상 지급 관리

---

## 🔧 개발 환경 설정

### 백엔드
```bash
cd /Users/ted/komission/backend
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
uvicorn app.main:app --reload --port 8000
```

로컬 코칭 WS 스모크 테스트:
```bash
scripts/smoke_coaching_ws.sh
```
기본값은 `SKIP_GEMINI_LIVE=1`로 Gemini Live 연결을 생략하며, `session_status=recording`까지 확인합니다.

백엔드 테스트:
```bash
backend/venv/bin/pytest
```

### 인증 (Google OAuth + JWT)
- 프로덕션 로그인: `POST /api/v1/auth/google`
- 로컬 테스트: `ALLOW_DEV_LOGIN=true`일 때만 `POST /api/v1/auth/token` 사용
- 현재 사용자: `GET /api/v1/auth/me`
- 토큰 갱신: `POST /api/v1/auth/refresh`
- 로그아웃: `POST /api/v1/auth/logout` (클라이언트에서 토큰 폐기)
- 필수 환경변수: `GOOGLE_CLIENT_ID`, `JWT_SECRET_KEY` (비개발 환경)

### 권한/관리자
- 기본 운영: `users.role = 'admin'` (DB role 기반)
- 부트스트랩/긴급: `SUPER_ADMIN_EMAILS` 환경변수
- 상세 절차는 `docs/08_OPERATIONS_RUNBOOK.md` 참고

### 프론트엔드
```bash
cd /Users/ted/komission/frontend
bun install
bun run dev  # localhost:3000
```
로케일 메시지는 `frontend/messages/{locale}.json`에 있으며 `NEXT_LOCALE` 쿠키로 전환됩니다 (ko/en).

### 모바일 ✅
```bash
cd /Users/ted/komission/mobile
npm install
npx expo prebuild --platform ios
npx expo run:ios --device
```

---

## 🔌 공유 인터페이스

### WebSocket 엔드포인트
`session_id`는 `POST /api/v1/coaching/sessions` 응답에서 획득합니다.
```
ws://localhost:8000/api/v1/ws/coaching/{session_id}
```

### 주요 메시지 타입
```typescript
// 서버 → 클라이언트
type Feedback = {
  type: 'feedback';
  message: string;
  audio_b64?: string;
  rule_id?: string;
  priority?: 'low' | 'medium' | 'high' | 'critical';
};

// Phase 2: 프레임 ACK
type FrameAck = {
  type: 'frame_ack';
  frame_t: number;  // 클라이언트 타임스탬프
  codec: 'h264' | 'jpeg';
};

// 클라이언트 → 서버
type Control = { type: 'control'; action: 'start' | 'stop' | 'pause' };
type VideoFrame = {
  type: 'video_frame';
  frame_b64: string;
  t_sec: number;
  t_ms?: number;     // Phase 2: RTT 측정용
  codec?: string;    // Phase 2: h264/jpeg
};
```

---

## 📋 현재 상태 (2026-01-02)

### ✅ 완료된 작업 (Week 1)

**모바일 앱 (Phase 1 + Phase 2)**
- ✅ H.265/H.264 코덱 지원
- ✅ 프레임 레이트 안정화
- ✅ 배터리/네트워크/저장공간 모니터링
- ✅ 적응형 화질 조정
- ✅ H.264 스트리밍 최적화 (50% 지연시간 감소)
- ✅ 음성/텍스트 코칭 토글
- ✅ DB 세션 저장 (RL 준비)

**백엔드**
- ✅ frame_ack 응답 (RTT 측정)
- ✅ H.264 코덱 지원
- ✅ client_t 에코

**확장 슬롯 (Phase 2+)**
- ⬜ 구도 가이드 (삼분법, 황금비)
- ⬜ 조명 추천
- ⬜ 미장센 추천

---

## 📞 협업 규칙

1. **브랜치 전략**: `feature/mobile-*` vs `feature/web-*`
2. **충돌 영역**: 백엔드 API 변경 시 상호 공유
3. **공유 훅**: `useCoachingWebSocket` 변경 시 양쪽 동기화
4. **PR 리뷰**: 백엔드 변경은 상호 확인 필수

---

## 📚 참고 문서

- VDG 2-Pass 프로토콜: `/docs/vdg_v4_2pass_protocol.md`
- 병렬 개발 전략: `/docs/21_PARALLEL_DEVELOPMENT_STRATEGY.md`
- 코칭 WebSocket: `/backend/app/routers/coaching_ws.py`
- MCP 설정: `/docs/MCP_CLAUDE_DESKTOP_SETUP.md`
